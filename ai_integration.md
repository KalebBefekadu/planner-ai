...
The current Groq Route Handlers are prototypes. Before further feature work they require:

- authenticated user and Workspace checks;
- shared schema validation for body, type, length, and provider output;
- distributed per-user quotas and cost budgets;
- safe error mapping and privacy-safe logs;
- cancellation/debounce for repeated Socratic requests;
- durable transcription job/audio lifecycle;
- provider adapters rather than module-level direct clients;
- tests for malformed JSON, timeouts, oversized input, and unauthorized use.

The unused SMART endpoint should be disabled until it is called through an approved structured-analysis Operation.
