...
/* Searching is a document load, and a document load takes the editor with it.
   Three ways an edit can be in flight when that happens, and all three must
   survive: still waiting on its debounce timer, already sent and awaiting a
   response, and refused by the server. The clock is paused deliberately in two
   of them so the test exercises leaving *before* autosave starts rather than
   racing an 800ms timer against whatever the machine is doing. */

test('search preserves a Note edited before autosave begins', async ({ workspace }) => {
  const { page } = workspace;
  await goTo(page, '/notes');
...
test('search waits for an in-flight save and the newer draft', async ({ workspace }) => {
  ...
  // Hold the first save open at the network so a second, newer edit is queued
  // behind it. Search must wait for both, not just the one already sent.
...
test('search keeps a refused draft open and can retry it', async ({ workspace }) => {
  ...
  // A refused save must not navigate away from the words it failed to keep.
