...
export const test = base.extend<{ workspace: Workspace }>({
  workspace: async ({ page }, provide, testInfo) => {
    ...
    try {
      ...
      // fails or returns no row. Request the setup route explicitly so an
      // unrelated journey cannot fail before it starts. Default redirect
      // behavior remains covered by the onboarding journey.
