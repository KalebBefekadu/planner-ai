...
-- not a reason to drop it: it is still the owner's choice, and a restore that
...
alter table public.note_import_items
  ...
  -- owner can edit, and a value that would be refused at the end should be
  -- refused before anything is committed rather than aborting a part-done
  -- import.
...
begin
  ...
  if p_operation_id = 'note.import-preview.v1' then
    ...
    insert into public.note_import_items (
      ...
      source_sort_key, source_icon_emoji, source_cover_key, source_cover_position,
      source_favorited_at
    )
    with recursive item as (
      select
        ...
        case
          ...
          then (source.appearance ->> 'coverPosition')::smallint
          else null
        end as source_cover_position,
        case
          when jsonb_typeof(source.appearance -> 'favoritedAt') = 'string'
          then (source.appearance ->> 'favoritedAt')::timestamptz
          else null
        end as source_favorited_at
    ...
    select
      ...
      item.source_icon_emoji,
      item.source_cover_key,
      item.source_cover_position,
      item.source_favorited_at
    from item
    ...
    loop
      ...
      insert into public.notes (
        ...
        icon_emoji, cover_key, cover_position, favorited_at
      ) values (
        ...
        v_item.source_icon_emoji, v_item.source_cover_key,
        -- The column's own default, restated rather than referenced, so a null
        -- staged position means "the default" instead of failing the not-null.
        coalesce(v_item.source_cover_position, 50), v_item.source_favorited_at
