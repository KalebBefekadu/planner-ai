-- A restored vault kept its hierarchy but not its order. Export records each
-- Note's sort_key in the manifest and the import discarded it, so every
-- restored Note was renumbered by staging order and siblings came back in an
-- order the owner never chose. Carry the recorded key through staging to the
-- Note.
--
-- The staging column is numeric(24, 12) to match notes.sort_key exactly.
-- Reordering a Note writes the midpoint between its neighbours, so a real
-- vault carries fractional keys that an integer column would silently
-- truncate into ties.
