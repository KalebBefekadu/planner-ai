...
create table public.action_templates (
  ...
  description_markdown text check (description_markdown is null or char_length(description_markdown) <= 50000),
...
create index action_templates_workspace_idx
on public.action_templates (workspace_id, status, next_occurrence_on)
where archived_at is null;
create trigger action_templates_set_updated_at before update on public.action_templates
for each row execute function public.set_updated_at();

alter table public.actions add constraint actions_recurrence_template_workspace_fk
foreign key (recurrence_template_id, workspace_id)
references public.action_templates(id, workspace_id);
create unique index actions_recurrence_occurrence_idx
on public.actions (workspace_id, recurrence_template_id, scheduled_on)
where recurrence_template_id is not null and scheduled_on is not null;

alter table public.action_templates enable row level security;
alter table public.action_templates force row level security;
create policy action_templates_owner_select on public.action_templates
for select using (
  exists (
    select 1 from public.workspaces workspace
    where workspace.id = action_templates.workspace_id
      and workspace.owner_user_id = auth.uid()
  )
);
revoke all on public.action_templates from public, anon, authenticated;
...
insert into public.operation_contracts (operation_id, risk_class, exposures, reversible) values
  ('action-template.create.v1', 'low', array['ui', 'chat', 'mcp'], true),
  ('action-template.update.v1', 'low', array['ui', 'chat', 'mcp'], true),
  ('action-template.status.v1', 'low', array['ui', 'chat', 'mcp'], true),
  ('action-template.materialize.v1', 'low', array['ui', 'chat', 'mcp', 'automation'], true),
  ('action-template.archive.v1', 'medium', array['ui', 'chat', 'mcp'], true)
...
begin
  ...
  if p_operation_id in ('action-template.create.v1', 'action-template.materialize.v1') then
    ...
    while v_occurrence_on <= v_through_on loop
      ...
      v_occurrence_on := case when v_template.cadence = 'weekly' then v_occurrence_on + 7
        else (date_trunc('month', v_occurrence_on)::date + interval '1 month'
          + (extract(day from v_occurrence_on)::integer - 1) * interval '1 day')::date end;
    end loop;
...
rename to dispatch_trusted_operation_action_template_base;
...
as $$
begin
  if p_operation_id like 'action-template.%' then
    return public.execute_action_template_operation(
      p_operation_id, p_input, p_idempotency_key, p_surface
    );
  end if;
  return public.dispatch_trusted_operation_action_template_base(
    p_operation_id, p_input, p_idempotency_key, p_surface
  );
end;
$$;
...
rename to build_mcp_workspace_snapshot_action_template_base;
...
as $$
  select coalesce(
    public.build_mcp_workspace_snapshot_action_template_base(p_workspace_id),
    '{}'::jsonb
  ) || jsonb_build_object(
    'actionTemplates', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', template.id,
          ...
          'nextOccurrenceOn', template.next_occurrence_on,
          'version', template.version
        ) order by template.next_occurrence_on, template.created_at
      )
      from public.action_templates template
      where template.workspace_id = p_workspace_id and template.archived_at is null
    ), '[]'::jsonb)
  );
$$;

alter function public.execute_operation_undo(text, jsonb, text, text)
rename to execute_operation_undo_action_template_base;
...
revoke all on function public.dispatch_trusted_operation_action_template_base(text, jsonb, text, text)
from public, anon, authenticated, service_role;
revoke all on function public.dispatch_trusted_operation(text, jsonb, text, text)
from public, anon, authenticated, service_role;
revoke all on function public.build_mcp_workspace_snapshot_action_template_base(uuid)
from public, anon, authenticated, service_role;
revoke all on function public.build_mcp_workspace_snapshot(uuid)
from public, anon, authenticated, service_role;
revoke all on function public.execute_operation_undo_action_template_base(text, jsonb, text, text)
from public, anon, authenticated, service_role;
