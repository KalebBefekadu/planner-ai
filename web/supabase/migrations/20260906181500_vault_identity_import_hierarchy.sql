-- An exported Planner AI vault records hierarchy by Note identity rather than by
-- folder nesting, so its parent paths do not end in '/' and do not prefix their
-- children. The import rejected every such vault outright, and its staging order
-- could create a child before its parent. Accept identity-based parentage,
-- require every item to descend from a root so parentage cannot form a cycle,
-- and stage items in true hierarchy order.
