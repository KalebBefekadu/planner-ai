-- A Notion export links one page to another by relative file path. Those paths
-- only exist inside the archive, so every internal link arrived in Planner AI
-- as dead text pointing at a file the workspace has never had. The owner's page
-- graph -- the part of a Notion workspace that is hardest to rebuild by hand --
-- was silently flattened into prose.
--
-- The import can resolve those links while it reads the bundle, but not to a
-- Note: no Note exists until the owner commits. So the preview stages a
-- resolved link as a marker carrying the SHA-256 of the target's source path,
-- and the commit replaces each marker with the created Note's address once the
-- ID exists. The digest is used instead of the path because Notion paths hold
-- spaces and parentheses, which cannot be written inside a Markdown link, and
-- because a fixed-length digest cannot be a prefix of another marker.
--
-- Resolution runs after every batch rather than only at the end, so a commit
-- that is interrupted and resumed still leaves no marker behind. Nothing else
-- about staging or committing changes: the raw source text stays in
-- note_import_items, and only the created Note's body is rewritten.
...
begin
  ...
  if p_operation_id = 'note.import-preview.v1' then
    ...
    -- Every marker whose target has been created by now becomes that Note's
    -- address. Items still uncommitted are resolved by a later batch, so no
    -- ordering between linking and linked pages is required.
    for v_link in
      select item.source_path, item.target_note_id
      from public.note_import_items item
      where item.workspace_id = v_workspace_id and item.job_id = v_job_id
        and item.target_note_id is not null
    loop
      update public.notes note
      set body_markdown = replace(
        note.body_markdown,
        'planner-ai-import://'
          || encode(extensions.digest(v_link.source_path, 'sha256'), 'hex'),
        '/notes?note=' || v_link.target_note_id::text
      )
      where note.workspace_id = v_workspace_id
        and note.id in (
          select target.target_note_id from public.note_import_items target
          where target.workspace_id = v_workspace_id and target.job_id = v_job_id
            and target.target_note_id is not null
        )
        and note.body_markdown like
          '%planner-ai-import://'
            || encode(extensions.digest(v_link.source_path, 'sha256'), 'hex') || '%';
    end loop;
  end if;
