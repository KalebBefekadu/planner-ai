-- note_import_jobs.status has always permitted 'canceled' and nothing has ever
-- written it. Dismissing the import dialog on a job in 'preview' left it in
-- 'preview' forever, so the next time the dialog opened it came back as the
-- active job -- the owner's own abandoned experiment, presented to them as an
-- import they still had to finish. Deciding not to import something is a real
-- decision, and the product had no way to record it.
--
-- This adds note.import-cancel.v1: one Operation that moves a job from
-- 'preview' to 'canceled'. It refuses a job that has already created a Note,
-- so cancelling can never be confused with undoing an import, and a cancelled
-- job can never be committed afterwards because the commit only ever accepts
-- 'preview' or 'committing'. The staged items are kept on purpose: the report
-- is how the owner sees what they chose not to import, and it stays readable.
