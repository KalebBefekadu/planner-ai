...
values ('trash.move.v1', 'inverse-operation')
...
rename to execute_operation_undo_capture_base;

create or replace function public.execute_trash_move_undo(
  p_operation_id text,
  p_input jsonb,
...
begin
  ...
  where receipt.id = (p_input ->> 'receiptId')::uuid
    ...
    and receipt.operation_id = 'trash.move.v1'
  ...
  v_batch_id := (v_original.result_json ->> 'batchId')::uuid;
  perform 1 from public.trash_batches
  where id = v_batch_id and workspace_id = v_workspace_id
    and restored_at is null and emptied_at is null
  for update;
  if not found then
  ...
  perform public.dispatch_trusted_operation(
    'trash.restore.v1', jsonb_build_object('batchId', v_batch_id), v_inverse_key, 'ui'
  );
  select id into v_inverse_receipt_id from public.operation_receipts
  where workspace_id = v_workspace_id and operation_id = 'trash.restore.v1'
...
begin
  ...
  if v_original_operation_id = 'trash.move.v1' then
    return public.execute_trash_move_undo(
      p_operation_id, p_input, p_idempotency_key, p_surface
    );
  end if;
  return public.execute_operation_undo_capture_base(
    p_operation_id, p_input, p_idempotency_key, p_surface
  );
exception
...
revoke all on function public.execute_operation_undo_capture_base(text, jsonb, text, text)
from public, anon, authenticated, service_role;
revoke all on function public.execute_trash_move_undo(text, jsonb, text, text)
from public, anon, authenticated, service_role;
