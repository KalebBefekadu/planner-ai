-- A CSV export is the one supported source whose meaning changes on the way
-- in: each row becomes a standalone Note, and the database it came from --
-- its columns as a schema, its formulas, relations and views -- does not
-- survive. The import reported nothing about that. Every converted row was
-- staged with disposition 'create' and a null reason, so the pre-commit report
-- looked exactly like a Markdown page that had been carried over intact, and
-- the owner had no way to see the conversion before agreeing to it.
--
-- The reason column already carries why an item got its disposition, so a
-- created item can carry the reason it is not a faithful copy. Staging now
-- accepts a per-item conversionNotice and stores it as the reason whenever the
-- item is neither unsupported nor a duplicate. Nothing is dropped or altered
-- by this: the same rows become the same Notes, and what changes is that the
-- report says out loud what the conversion cost.
