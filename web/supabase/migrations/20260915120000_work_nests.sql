...
-- means something exact. actions.parent_action_id has always been a plain
-- self-referencing foreign key with no constraint tying a child to a different
-- horizon, and the database already walks it to arbitrary depth -- archive,
-- trash and undo all cascade through a whole subtree with recursive CTEs.
--
-- The interface is what assumed one level. It spent the only relationship the
-- schema offers on the monthly-to-weekly rollup and then read the same column
-- straight back out as "the monthly one", so there was no way to put a task
-- under a task. This is the same shape as the Notes tree before it got
-- disclosure controls: the storage was always a tree and the screen flattened
-- it.
--
-- Two guards make it safe to open up, and neither existed before because
-- nothing could reparent an Action under another Action:
--
--   * a depth bound, because the recursive cascades need a predictable cost.
--     Four levels is more than the owner's own Notion export uses.
--   * a cycle guard. Notes have had one since the first migration. Without it
--     an Action can be made its own ancestor and every cascade over
--     parent_action_id fails to terminate.

begin;

-- One definition of the bound, in the database as well as in
-- src/lib/planner/nesting.ts.
create or replace function public.action_max_depth()
returns integer
language sql
immutable
parallel safe
...
-- How many Action ancestors this Action has. Zero for a root. The internal
-- guard is against a cycle that predates the cycle guard below: this function
-- has to terminate even on data it is being used to prevent.
create or replace function public.action_ancestor_depth(p_workspace_id uuid, p_action_id uuid)
returns integer
language sql
stable
...
as $$
  with recursive up as (
    select action.id, action.parent_action_id, 0 as depth
    from public.actions action
    where action.id = p_action_id and action.workspace_id = p_workspace_id
    union all
    select parent.id, parent.parent_action_id, up.depth + 1
    from public.actions parent
    join up on parent.id = up.parent_action_id
    where parent.workspace_id = p_workspace_id and up.depth < 32
  )
  select coalesce(max(depth), 0) from up;
$$;

-- How far the deepest leaf sits below this Action. Zero for a leaf.
create or replace function public.action_subtree_height(p_workspace_id uuid, p_action_id uuid)
returns integer
language sql
stable
set search_path = pg_catalog, public
as $$
  with recursive down as (
    select action.id, 0 as height
    from public.actions action
    where action.id = p_action_id and action.workspace_id = p_workspace_id
    union all
    select child.id, down.height + 1
    from public.actions child
    join down on child.parent_action_id = down.id
    where child.workspace_id = p_workspace_id and down.height < 32
  )
  select coalesce(max(height), 0) from down;
$$;

-- Whether p_candidate sits anywhere inside the subtree rooted at p_action_id,
-- the Action included. Reparenting onto one's own descendant is the cycle.
create or replace function public.action_has_descendant(
  p_workspace_id uuid,
  p_action_id uuid,
  p_candidate uuid
) returns boolean
...
as $$
  with recursive down as (
    ...
    from public.actions child
    join down on child.parent_action_id = down.id
    where child.workspace_id = p_workspace_id and down.depth < 32
  )
  select exists (select 1 from down where id = p_candidate);
$$;

-- Reachable only from the security-definer operation executors, which is where
-- every write to parent_action_id goes.
revoke all on function public.action_max_depth() from public, anon, authenticated;
revoke all on function public.action_ancestor_depth(uuid, uuid) from public, anon, authenticated;
revoke all on function public.action_subtree_height(uuid, uuid) from public, anon, authenticated;
revoke all on function public.action_has_descendant(uuid, uuid, uuid)
      ...
      or (v_kind = 'week' and (
        v_parent_id is null or not exists (
          ...
          join public.planning_horizons horizon
            on horizon.id = parent.horizon_id and horizon.workspace_id = parent.workspace_id
          where parent.id = v_parent_id and parent.workspace_id = v_workspace_id
            and parent.goal_id = v_goal_id and horizon.kind in ('month', 'week')
            and parent.archived_at is null and parent.trashed_at is null
        )
      )) then
