...
-- list keeps the order the owner built it in rather than reshuffling on every
-- edit, and so Undo can restore the exact prior instant instead of guessing
-- one. Re-marking an existing favourite keeps its original instant, so a stray
-- second click cannot silently move it in the list. Writes go
...
create index notes_favorited_at_idx
  on public.notes (workspace_id, favorited_at)
