...
select is(
  (select public from storage.buckets where id = 'note-attachments'),
  false,
  'the Notes attachment bucket is private'
);
...
select lives_ok(
  $$select public.execute_ui_operation(
    'note.create.v1',
    '{"title":"Attachment evidence","bodyMarkdown":"","parentNoteId":null}',
...
select throws_ok(
  ...
  '42501', null,
  'an authenticated user cannot create attachment metadata directly'
);
