...
Status: pre-beta working runbook, 2026-08-21. Evidence marked pending is a release blocker, not an assumed success.
...
Before cutover, export a logical backup, record table counts, inspect migration drift, and verify the backup can be read. Apply only files in `web/supabase/migrations` in order; never use `database/schema.sql` or dashboard copy/paste SQL. Run `npm run test:db`, regenerate `src/types/supabase.generated.ts`, and compare post-migration counts and ownership checks.
...
PLANNER_PRODUCTION_DATABASE_URL='postgresql://postgres:<database-password>@db.<project-ref>.supabase.co:5432/postgres'
...
| ---------------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Remote database backup and restore       | Pending | Management inventory found one Auth user, six legacy tables, and one Transcript; no backup/restore evidence                                        |
| Private Storage backup and restore       | Pending | Attachment infrastructure not enabled                                                                                                              |
| Canonical migrations and pgTAP           | Pending | Stage-0 security applied and verified; production canonical cutover requires explicit approval plus backup/rollback evidence or an approved branch |
