...
Status: **Approved pre-implementation architecture.** See `planner_ai_decision_register.md` and `docs/adr/` for rationale.
...
- Domain rows are Workspace-scoped relational entities defined in `database_schema.md`.
...
- Physical changes use Supabase CLI migrations. `database/schema.sql` is not a migration source.
...
The existing `planner` directory is an integration lab for studying patterns. It is not shipped and must not receive production data.
